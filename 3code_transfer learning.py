...
Method: 
Fuqing cohort assists other cohort in modeling,
FEV1 modeling using top six features, 
FVC  modeling using top seven features.
"""
...
#main process
data_fq, x_fq, y1_fq, y2_fq = load_data("fuqing.csv")
sorted_index_remained_fq = delete_noise(data_fq,0.99)
x_fq = x_fq[sorted_index_remained_fq,:]
y1_fq = y1_fq[sorted_index_remained_fq]
y2_fq = y2_fq[sorted_index_remained_fq]
model1 = GradientBoostingRegressor(random_state=seed).fit(x_fq, y1_fq)
model2 = GradientBoostingRegressor(random_state=seed).fit(x_fq, y2_fq)
feature_sort_index1 = np.argsort(-model1.feature_importances_)[0:6]
feature_sort_index2 = np.argsort(-model2.feature_importances_)[0:7]
x_fq1 = x_fq[:,feature_sort_index1]
x_fq2 = x_fq[:,feature_sort_index2]

data,x,y1,y2 = load_data("zhuhai.csv") #this can be replaced with Wuhan data
sorted_index_remained = delete_noise(data,0.99)
x1 = x[sorted_index_remained]
x1 = x1[:,feature_sort_index1]
x2 = x[sorted_index_remained]
x2 = x2[:,feature_sort_index2]
...
rkf = RepeatedKFold(n_splits=n_splits, n_repeats=5, random_state=seed) #, random_state=seed
for i, (train_index, test_index) in enumerate(rkf.split(x1)):
    x_train1 = x1[train_index]
    ...
    x_train1 = np.vstack([x_train1, x_fq1])
    y_train1 = np.hstack([y_train1, y1_fq])
    x_train2 = np.vstack([x_train2, x_fq2])
    y_train2 = np.hstack([y_train2, y2_fq])
                
    model1=GradientBoostingRegressor(random_state=seed).fit(x_train1, y_train1)
    model2=GradientBoostingRegressor(random_state=seed).fit(x_train2, y_train2)
